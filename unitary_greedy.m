function score_adaptive = unitary_greedy(no, k_copies, n_monte_carlo)
...
%         for zk=1:nh
%             k = k+1;
%             p_initial(k,1) = pk(xk,1)*pk(yk,2)*pk(zk,3); 
%         end
%     end
% end         
...
for kx=1:nh
    for ky=1:nh
        for kz=1:nh
            k = k+1;
            Ck(:,:,k) = kraus2choi(expm(1i*(theta_k(kx,1)*X+theta_k(ky,2)*Y+theta_k(kz,3)*Z)));     % Choi of the true parameter theta_k
        end 
    end
end
...
for mc = 1:n_monte_carlo
    ...
    for copy = 1:k_copies
        ...
        T_temp = T_adaptive;
        gap        = 1;
        precision  = 1e-4; %tune to 1e-6 when more precision is needed
        rounds     = 0;
        ...
        while gap > precision
            ...
            [estimators, ~] = estimator_optimization(p_current, T_temp, Ck, theta_i);        % optimization over the estimators
            theta_i = estimators;
