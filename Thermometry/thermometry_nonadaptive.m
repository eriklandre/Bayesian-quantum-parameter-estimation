function score_nonadaptive = thermometry_nonadaptive_new(No, k_copies, n_monte_carlo, t)
...
theta_k(1:Nh) = Tmin:(Tmax-Tmin)/(Nh-1):Tmax;   % hypothesis grid (column)
% p_initial(1:Nh,1) = 1/Nh;            % uniform prior
% mu_T    = 1.0;
% sigma_T = 0.25;

% logp = -0.5*((theta_k(:) - mu_T)/sigma_T).^2; % Gaussian truncated prior
% logp = logp - max(logp);
% p_initial = exp(logp);
% p_initial = p_initial / sum(p_initial);
p_initial = normpdf(theta_k(:),1,0.6);
p_initial = p_initial ./ sum(p_initial);

cache_dir = sprintf('thermo_cache_nonadaptive_new/No=%d_k=%d_Nh=%d_t=%.6f_eps=%.6f_g=%.6f', No, k_copies, Nh, time_value, eps_val, g_val); % this creates the cache folder where all the data will be stored
...
for i1 = 1:No
    ...
    theta2 = estimator_optimization(p1, T1, Ck, theta_k); % greedy solve for copy-2 given this posterior

    save(branch_file, 'theta2', 'p1', '-v7.3'); % save the copy-2 branch data for this i1
...
traj = zeros(n_monte_carlo, 2);  % store outcomes [i1 i2] (optional)
...
parfor mc = 1:n_monte_carlo
    ...
    C_true = ChoiOperatorThermo(theta_true, eps_val, g_val, time_value); % true channel choi at theta_true
    ...
    probs2 = outcome_probs(T1_loc, C_true); 
...
results_dir = 'results_files_nonadaptive_thermometry_new'; % Save results to a separate folder 
...
outfile = fullfile(results_dir, sprintf('thermometry_nonadaptive_greedy_k2_cached_t=%.6f_No=%d_Nh=%d.mat', time_value, No, Nh));
save(outfile, 'results', '-v7.3');

fprintf('Non-adaptive (cached, k=2) score: %.6f\n', score_nonadaptive);
...
% One greedy round: optimize tester (SDP) and refine estimators via seesaw
d  = 2;
Nh = length(p);
...
% initial tester
[T_temp, score_old, ~] = testeroptimization_sdp_kcopy_seesaw(Xi, [d d], 1, 2, -1);
...
while gap > precision
    theta_i = estimator_optimization(p, T_temp, Ck, theta_k);
    ...
    [T_new, score_new, ~] = testeroptimization_sdp_kcopy_seesaw(Xi, [d d], 1, 2, -1);
...
% Posterior update: p_new(k) ∝ p(k) * Tr[T_outcome * Ck(k)]
Nh = length(p);
lik = zeros(Nh,1);
...
% probs(i) = Tr[T_i * C_true], normalized and clipped
No = size(T,3);
probs = zeros(No,1);
for i = 1:No
