...
% p: damping parameter of the channel
...
for no=no_initial:no_final
        ...
        for ky = 1:nh
            for kz = 1:nh
                ...
                if r <= pi      % Only keep r <= pi (boundary has zero measure)
                    ...
                    KrausT{1,1} = A0*U; 
                    KrausT{2,1} = A1*U;
                    Ck(:,:,k) = ChoiMatrix(KrausT);   % Choi of the composed channel    
                else
                    p_initial(k) = 0;
                end
            end
    ...
    for i=1:No
        for k=1:Nh
            ...
            Xi(:,:,i) = Xi(:,:,i) + p_initial(k)*r(i,k)*Tensor(Ck(:,:,k),ncopies);      % computing the Xi's
        end
    end  
    ...
    if method==3    
        ...
        while gap>precision
            ...
            [estimators] = estimator_optimization(p_initial,T_temp,Ck,theta_k,ncopies);     % optimization over the estimators
